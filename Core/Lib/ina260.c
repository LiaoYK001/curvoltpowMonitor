...
static ina260_identification_t INA260_DEVICE_ID = {.revision = 0x00,
                                                   .device = 0x227};
 ...
 */
int ina260_wait_until_ready(uint32_t timeout) { return ina260_ready(); }
...
int ina260_get_voltage(float *voltage) {
  ...
  if (ina260_device.i2c_read) {
    if (INA_STATUS_OK !=
        (status = ina260_device.i2c_read(INA260_REG_VOLTAGE, &v, 2U))) {
      *voltage = 0;
      return status;
    }
  }
...
int ina260_get_current(float *current) {
  ...
  if (ina260_device.i2c_read) {
    if (INA_STATUS_OK !=
        (status = ina260_device.i2c_read(INA260_REG_CURRENT, &u, 2U))) {
      *current = 0;
      return status;
    }
  }
